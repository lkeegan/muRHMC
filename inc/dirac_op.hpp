...
class dirac_op {
 ...
 public:
  ...
  template <int N>
  void D_eo(field<block_fermion_matrix<N> >& lhs,
            const field<block_fermion_matrix<N> >& rhs,
            const field<gauge>& U) const {
...
#pragma omp parallel for
  ...
  template <int N>
  void D_oe(field<block_fermion_matrix<N> >& lhs,
            const field<block_fermion_matrix<N> >& rhs,
            const field<gauge>& U) const {
...
#pragma omp parallel for
    for (int ix_o = 0; ix_o < lhs.V; ++ix_o) {
      ...
      for (int mu = 0; mu < 4; ++mu) {
        ...
        lhs[ix_o].noalias() +=
            -0.5 * U.dn(ix, mu)[mu].adjoint() * rhs.dn(ix, mu);
      }
    }
  }

  // Staggered massive Dirac operator D with isospin chemical potential mu_I:
  // lhs = D(rhs)
  template <int N>
  void D(field<block_fermion_matrix<N> >& lhs,
         const field<block_fermion_matrix<N> >& rhs, field<gauge>& U) {
    apply_eta_bcs_to_U(U);
    double mu_I_plus_factor = exp(0.5 * mu_I);
    double mu_I_minus_factor = exp(-0.5 * mu_I);
    // default static scheduling, with N threads, split loop into N chunks, one
    // per thread
    ////#pragma omp parallel for
  ...
  // If lhs and rhs are EVEN_ONLY fermion fields then this is the even-even
  // sub-block of D Ddagger for mu_I=0
  template <int N>
  void DDdagger(field<block_fermion_matrix<N> >& lhs,
                const field<block_fermion_matrix<N> >& rhs, field<gauge>& U,
                field<block_fermion_matrix<N> >& tmp,
                bool LEAVE_ETA_BCS_IN_GAUGE_FIELD = false) {
    // NOTE: tmp must be odd if EO sub-block used
    if (lhs.eo_storage == field<block_fermion_matrix<N> >::EVEN_ONLY &&
        rhs.eo_storage == field<block_fermion_matrix<N> >::EVEN_ONLY) {
  ...
  void DDdagger(field<block_fermion_matrix<N> >& lhs,
    ...
    if (lhs.eo_storage == field<block_fermion_matrix<N> >::EVEN_ONLY &&
      ...
      // if lhs and rhs are both even-only fields,
      // then use even-even sub block of operator (and assume mu_I=0)
      field<block_fermion_matrix<N> > tmp_o(
          rhs.grid, field<block_fermion_matrix<N> >::ODD_ONLY);
  ...
  // Returns phase angle of determinant of D by explicitly constructing and
  // diagonalising D
  double D_phase_angle(field<gauge>& U);

  // Returns real part of isospin density by explicitly constructing and
  // diagonalising DDdagger
  ...
  // in-place chebyshev polynomial of DDdag acting on fermion or block_fermion
  // fields Uses recursive formula from hep-lat/0512021 c_{n+1}= 2*z*c_{n} -
  // c_{n-1} where z = ((v+u) - 2 DDdag) / (u - v) c_0 = 1, c_1 = z
  template <typename fermion_type>
  void chebyshev(int k, double u, double v, field<fermion_type>& X,
                 field<gauge>& U) {
  ...
  double largest_eigenvalue_bound(
      field<gauge>& U,
