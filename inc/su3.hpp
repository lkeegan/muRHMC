...
EIGEN_DEFINE_STL_VECTOR_SPECIALIZATION(Eigen::FullPivLU<block_matrix>)
...
constexpr double exp_ch_c_n[exp_ch_N + 1] = {1.0,
                                             1.0,
                                             0.5,
                                             0.16666666666666666,
                                             ...
                                             7.6471637318198174e-13,
                                             4.7794773323873859e-14,
...
class SU3_Generators {
 ...
 public:
  ...
  SU3_Generators() {
    ...
    T_[0] << 0, 0.5, 0, 0.5, 0, 0, 0, 0, 0;

    T_[1] << 0, -0.5 * I, 0, 0.5 * I, 0, 0, 0, 0, 0;

    T_[2] << 0.5, 0, 0, 0, -0.5, 0, 0, 0, 0;

    T_[3] << 0, 0, 0.5, 0, 0, 0, 0.5, 0, 0;

    T_[4] << 0, 0, -0.5 * I, 0, 0, 0, 0.5 * I, 0, 0;

    T_[5] << 0, 0, 0, 0, 0, 0.5, 0, 0.5, 0;

    T_[6] << 0, 0, 0, 0, 0, -0.5 * I, 0, 0.5 * I, 0;

    T_[7] << 0.5 / sqrt(3.0), 0, 0, 0, 0.5 / sqrt(3.0), 0, 0, 0,
        -1.0 / sqrt(3.0);
  }
};
...
class gauge {
 ...
 public:
  ...
  bool operator==(const gauge& other) const {
    return (U_[0] == other[0]) && (U_[1] == other[1]) && (U_[2] == other[2]) &&
           (U_[3] == other[3]);
  }
