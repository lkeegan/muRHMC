...
Eigen::MatrixXcd dirac_op::dD_dmu_dense_matrix(field<gauge>& U) const {
  ...
  for (int ix = 0; ix < U.V; ++ix) {
    D_matrix.block<3, 3>(3 * ix, 3 * U.iup(ix, 0)) =
        mu_I * 0.25 * mu_I_plus_factor * U[ix][0];
    D_matrix.block<3, 3>(3 * ix, 3 * U.idn(ix, 0)) =
        mu_I * 0.25 * mu_I_minus_factor * U.dn(ix, 0)[0].adjoint();
  }
...
double dirac_op::pion_susceptibility_exact(field<gauge>& U) {
  ...
  return eigenvaluesDDdag.cwiseInverse().sum().real() /
         static_cast<double>(3 * U.V);
}

double dirac_op::largest_eigenvalue_bound(
    field<gauge>& U, field<fermion>::eo_storage_options EO_STORAGE,
    double rel_err) {
  ...
  while ((lambda_max_err / lambda_max) > rel_err) {
    ...
    // std::cout << "lambda_max" << lambda_max << ", " << lambda_max_err <<
    // std::endl;
  }
