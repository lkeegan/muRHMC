...
int main(int argc, char *argv[]) {
  ...
  */
  ...
  // just do reversibility measurements:
  /*
  for(int i=0; i<=run_pars.n_traj; ++i) {
          read_gauge_field (U, run_pars.base_name, run_pars.initial_config+i);
          hmc.trajectory (U, D, true, false);
  }
  exit(0);
  */
  ...
  for (int i = 1; i <= run_pars.n_therm; ++i) {
    ...
    std::cout << "# therm " << i << "/" << run_pars.n_therm
              ...
              << "\t dE: " << hmc.deltaE << std::endl;
  }
  log("Thermalisation acceptance",
      static_cast<double>(acc) / static_cast<double>(run_pars.n_therm));
  ...
  // print average, error & integrated autocorrelation time of measured
  // observables
  log("Acceptance",
      static_cast<double>(acc) / static_cast<double>(run_pars.n_traj));
  log("");
  std::cout << "# " << std::left << std::setw(20) << "observable"
            << "average\t\terror\t\t\ttau_int\t\terror" << std::endl;
