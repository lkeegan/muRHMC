...
//	log("N_shifts", N_shifts);
	...
	field<fermion> tmp_chi (grid, field<fermion>::EVEN_ONLY), tmp_phi(grid, field<fermion>::EVEN_ONLY);
	for(int i=0; i<N; ++i) {
		if(source_point) {
			// point source
			log("point source");
	...
	if (read_fermion_field(x0_star, fermion_filename)) {
		...
		double x0_star_res = is_field_equal(tmp_phi, chi[0])/chi[0].norm();	
