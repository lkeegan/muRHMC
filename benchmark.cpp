...
int main(int argc, char *argv[]) {
  if (argc - 1 != 1) {
    std::cout << "This program requires 1 argument, the lattice size L:"
              << std::endl;
  ...
  std::vector<field<fermion> > phi(
      N_rhs, field<fermion>(U.grid, field<fermion>::EVEN_ONLY));
  ...
  std::cout << N_rhs << "\t" << dirac_count / N_dirac_ops << "\t"
            << block_dirac_count / N_dirac_ops << std::endl;
  exit(0);
  ...
  typedef Eigen::Matrix<std::complex<double>, Eigen::Dynamic, N_rhs>
      fermion_block_field;
  ...
  std::cout << "# BLOCKAddBigmatRunTime: " << block_add_bigmat_count
            << std::endl;
  ...
  std::complex<double> *A_mkl = (std::complex<double> *)mkl_malloc(
      sizeofA * sizeof(std::complex<double>), 64);
  std::complex<double> *B_mkl = (std::complex<double> *)mkl_malloc(
      sizeofA * sizeof(std::complex<double>), 64);
  for (int in = 0; in < N_rhs; ++in) {
    for (int ix = 0; ix < bphi.V; ++ix) {
      for (int ic = 0; ic < 3; ++ic) {
  ...
  std::complex<double> *alpha_mkl = (std::complex<double> *)mkl_malloc(
      N_rhs * N_rhs * sizeof(std::complex<double>), 64);
  std::complex<double> *beta_mkl = (std::complex<double> *)mkl_malloc(
      N_rhs * N_rhs * sizeof(std::complex<double>), 64);
  ...
  for (int i = 0; i < N_adds; ++i) {
    // A_mkl += B_mkl alpha
    cblas_zgemm(CblasColMajor, CblasNoTrans, CblasNoTrans, 3 * bphi.V, N_rhs,
                N_rhs, &one, B_mkl, 3 * bphi.V, alpha_mkl, N_rhs, &one, A_mkl,
                3 * bphi.V);
    // B_mkl += A_mkl beta
    cblas_zgemm(CblasColMajor, CblasNoTrans, CblasNoTrans, 3 * bphi.V, N_rhs,
                N_rhs, &one, A_mkl, 3 * bphi.V, beta_mkl, N_rhs, &one, B_mkl,
                3 * bphi.V);
  }
  mkl_free(alpha_mkl);
  mkl_free(beta_mkl);
  ...
  std::cout << "# BLOCKAdd_mkl_bigmat_RunTime: " << block_add_mkl_bigmat_count
            << std::endl;
  ...
  mkl_zgepack_compact(MKL_COL_MAJOR, 3, N_rhs,
                      reinterpret_cast<const MKL_Complex16 *const *>(A_mkl_arr),
                      3, a_compact, 3, format, nm);
  mkl_zgepack_compact(MKL_COL_MAJOR, 3, N_rhs,
                      reinterpret_cast<const MKL_Complex16 *const *>(B_mkl_arr),
                      3, b_compact, 3, format, nm);
  mkl_zgepack_compact(
      MKL_COL_MAJOR, N_rhs, N_rhs,
      reinterpret_cast<const MKL_Complex16 *const *>(alpha_mkl_arr), N_rhs,
      alpha_compact, N_rhs, format, nm);
  mkl_zgepack_compact(
      MKL_COL_MAJOR, N_rhs, N_rhs,
      reinterpret_cast<const MKL_Complex16 *const *>(beta_mkl_arr), N_rhs,
      beta_compact, N_rhs, format, nm);

  for (int i = 0; i < N_adds; ++i) {
    // A_mkl += B_mkl alpha
    mkl_zgemm_compact(MKL_COL_MAJOR, (MKL_TRANSPOSE)CblasNoTrans,
                      (MKL_TRANSPOSE)CblasNoTrans, 3, N_rhs, N_rhs,
                      reinterpret_cast<const MKL_Complex16 *>(&one), b_compact,
                      3, alpha_compact, N_rhs,
                      reinterpret_cast<const MKL_Complex16 *>(&one), a_compact,
                      3, format, nm);
    // B_mkl += A_mkl beta
    mkl_zgemm_compact(MKL_COL_MAJOR, (MKL_TRANSPOSE)CblasNoTrans,
                      (MKL_TRANSPOSE)CblasNoTrans, 3, N_rhs, N_rhs,
                      reinterpret_cast<const MKL_Complex16 *>(&one), a_compact,
                      3, beta_compact, N_rhs,
                      reinterpret_cast<const MKL_Complex16 *>(&one), b_compact,
                      3, format, nm);
  }

  mkl_zgeunpack_compact(MKL_COL_MAJOR, 3, N_rhs,
                        reinterpret_cast<MKL_Complex16 *const *>(A_mkl_arr), 3,
                        a_compact, 3, format, nm);
  mkl_zgeunpack_compact(MKL_COL_MAJOR, 3, N_rhs,
                        reinterpret_cast<MKL_Complex16 *const *>(B_mkl_arr), 3,
                        b_compact, 3, format, nm);
  ...
  std::cout << "# BLOCKAdd_mkl_compact_RunTime: " << block_add_mkl_compact_count
            << std::endl;

  // data output
  std::cout
      << "# "
         "N_rhs\tN_dirac\tN_add\tdirac\tadd\tadd_mkl_bigmat\tadd_mkl_compact: "
      << std::endl;
  std::cout << N_rhs << "\t" << 2 * N_dirac_ops << "\t" << 2 * N_adds << "\t"
            << block_dirac_count << "\t" << block_add_count << "\t"
  ...
  // data output
  std::cout << "# N_rhs\tL\tdirac_time\tblock_time\tadd_time\tblock_time: "
            << block_dirac_count << std::endl;
  std::cout << N_rhs << "\t" << L << "\t" << dirac_count << "\t"
            << block_dirac_count << "\t" << add_count << "\t" << block_add_count
            << "\t" << std::endl;
